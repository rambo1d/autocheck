...
async function getWeather(returnHour = 15) {
  ...
  try {
    ...
    const rawCur = execSync('curl.exe -s "https://wttr.in/Moscow?format=temp:%t,feels:%f,cond:%C,precip:%p&lang=ru"', { encoding: 'utf8' });
    const parts = {};
    ...
    const tableStr = execSync('curl.exe -s "https://wttr.in/Moscow?1TFq&lang=ru"', { encoding: 'utf8' });
